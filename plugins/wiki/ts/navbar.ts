 ...
 */
module Wiki {
  _module.controller("Wiki.NavBarController", ["$scope", "$location", "$routeParams", "wikiBranchMenu", ($scope, $location, $routeParams, wikiBranchMenu:BranchMenu) => {
    ...
    $scope.sourceLink = () => {
      ...
      angular.forEach(Wiki.customViewLinks($scope), (link) => {
        if (path.startsWith(link)) {
          answer = <string>Core.createHref($location, Wiki.startLink($scope) + "/view" + path.substring(link.length))
        }
      });
